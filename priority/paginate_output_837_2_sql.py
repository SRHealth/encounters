...
def write_encounter_file(dataframes, output_directory="/Users/stephanielaface/Documents/GitHub/encounters/output"):
    """
    ...
    output_file_path = os.path.join(output_directory, f"CSV_TCHP_PROF_TX_{timestamp}.DAT")
...
def main():
    ...
    if not test_connection(connection_params):
        logging.error("Connection test failed. Please check your credentials.")
        return
    
    try:
        # Read all encounter tables
        dataframes = read_all_encounter_tables(connection_params)
