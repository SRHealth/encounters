WITH transformed_data AS (
    ...
    SELECT 
        ...
        primary_ride_id
    FROM srh_encounters.sunflower_encountered_member_date_rides_test
    ...
    SELECT 
        ...
        ROUND(daily_grand_total * (CASE WHEN daily_ride_count = 1 THEN 10.0 ELSE 20.0 END / total_daily_distance), 2) AS totalallowedamt,
        primary_ride_id
    FROM srh_encounters.sunflower_encountered_member_date_rides_test
    WHERE procedure_code NOT IN ('A0190', 'A0110', 'A0180') 
      AND ((daily_ride_count = 1 AND total_daily_distance > 10) 
    ...
    SELECT 
        ...
        ROUND(daily_grand_total * ((total_daily_distance - (CASE WHEN daily_ride_count = 1 THEN 10.0 ELSE 20.0 END)) / total_daily_distance), 2) AS totalallowedamt,
        primary_ride_id
    FROM srh_encounters.sunflower_encountered_member_date_rides_test
    WHERE procedure_code NOT IN ('A0190', 'A0110', 'A0180') 
      AND ((daily_ride_count = 1 AND total_daily_distance > 10) 
...
JOIN grand_totals g ON n.primary_ride_id = g.primary_ride_id
ORDER BY 
    n.primary_ride_id,
    n.row_num;